...
const roomsSlice = createSlice({
  ...
  initialState,
  reducers: {
    addRoom: (state: IStateRoom, action: PayloadAction<{ data: IRoomWithQuantity; quantity: number }>) => {
      state.rooms = [...state.rooms, action.payload]
    },

    deleteRoom: (state: IStateRoom, action: PayloadAction<IRoom>) => {
      state.rooms = state.rooms.filter((room) => room.data.id !== action.payload.id)
    },
    ...
    decreaseRoom: (state: IStateRoom, action: PayloadAction<IRoom>) => {
      const existRoom = state.rooms.find((room) => room.data.id === action.payload.id)
