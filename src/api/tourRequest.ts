...
const getAllTour = createAsyncThunk<
  ...
  { rejectValue: string }
>('tour/getAll', async ({ location = '', types = [], durations, price, start = 0, limit = 6 }, { rejectWithValue }) => {
  try {
    const separateType = () => {
      let result = ''
      types.forEach((v) => {
    ...
    const arrDuration = durations && durations.length ? durations.split('-') : [0, 30]
    ...
    localStorage.setItem('totalTour', JSON.stringify(parseInt(res.headers['x-total-count'])))
    return res.data
  } catch (error) {
...
const getTourById = createAsyncThunk<ITour, number, { search: string; rejectValue: string }>(
  'tour/getById',
...
const getTours = async ({ _page = 1, _limit = 10 }: { _page?: number; _limit?: number }): Promise<ITour[]> => {
...
const getLocationTour = async ({ data }: { data: string }): Promise<ITour[]> => {
  const res = await instanceLocal(`/tours?location_like=${data}`)
  return res.data
}

export { getAllTour, getTourById, getFiltersTour, getTours, changeFavoriteTour, getLocationTour }
