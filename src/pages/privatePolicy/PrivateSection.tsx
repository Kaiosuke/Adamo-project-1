        ...
        <section className=" max-w-[880px] w-full">
          <div className="italic mt-4">
            <p>
              ...
              “Privacy Policy” or “Policy”) outlining Ojimah’s practices regarding the collection, use, preservation and
              disclosure of personal information collected in accordance with applicable laws.
            </p>
            <br />
            <p>
              When You use Ojimah’s services, whether to make searches, sign in to receive offers and news, or purchase
              any service through any other of its sale channels, call center, website (hereinafter, the “Website”) or
              application for mobiles and tablets (hereinafter, the “Application”) you trust information to us. This
              Privacy Policy seeks to help you understand which data we collect, why we do it, and what we do with such
              data. You, as a user, accept the practices detailed below, so we recommend you to carefully read the
            ...
            <ul className="list-decimal not-italic ml-4 mt-6 flex flex-col gap-2">
              ...
              <li className="pl-4">2. What do we use the information collected for?</li>
              <li className="pl-4">3. Who we share the information collected with?</li>
              <li className="pl-4">4. Where do we store and how we protect the information collected?</li>
              <li className="pl-4">5. How can the information collected be accessed, deleted and/or updated?</li>
              <li className="pl-4">6. How does Despegar interact with your social networks?</li>
            </ul>
          </div>
          <div>
            {privatePolicy.map((value, index) => (
              <div key={index} className="md:mt-8 mt-6 text-four">
                ...
                {value.quickDes ? <div className="my-4">{value.quickDes}</div> : null}
                {value.contents ? (
                  <div>
                    {value.contents.map((content, index) => (
                      <div className="md:mb-10 mb-8" key={index}>
                        <h3 className="text-size-xl text-secondary mt-8">{content.title}</h3>
                        <div>
                          {handleSeparateWord(content.description).map((cont, index) => (
